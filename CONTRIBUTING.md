...
- Open the `examples/react-native-app/ios/TrackAsiaReactNativeExample.xcworkspace` file
  - Shows the library as `Pods > Development Pods > trackasia-react-native`
  - Shows the React Native example app as `TrackAsiaReactNativeExample`
...
1. Run `yarn pack --out %s-%v.tgz` within the library<br/>
   This will create a `@trackasia-trackasia-react-native-X.X.X.tgz` file for latest version at the root of the
   library
2. Copy the file into your app project, you can choose any path or name
3. Install from file<br/>
   `yarn add ./@trackasia-trackasia-react-native-X.X.X.tgz` or similar command of the package manager of you choice

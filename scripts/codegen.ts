import trackasiaGlStyleSpec from "@track-asia/trackasia-gl-style-spec/src/reference/latest";
import ejs from "ejs";
...
const TEMPLATE_MAPPINGS = [
  ...
  {
    input: path.join(TEMPLATES_PATH, "TrackAsiaRNStyles.ts.ejs"),
    output: path.join(JS_OUTPUT_PATH, "types", "TrackAsiaRNStyles.ts"),
  },
  {
...
async function generate() {
  ...
  function getPropertiesForLight() {
    const lightAttributes = trackasiaGlStyleSpec.light;
  ...
  function getPropertiesForLayer(layerName: string) {
    const paintAttributes = trackasiaGlStyleSpec[`paint_${layerName}`];
    const layoutAttributes = trackasiaGlStyleSpec[`layout_${layerName}`];
    ...
    return props.filter((prop) => {
      ...
      // https://github.com/track-asia/trackasia-react-native/issues/562
      return !["textVariableAnchorOffset"].includes(prop.name);
    });
  }

  function getSupportedLayers() {
    return Object.entries(trackasiaGlStyleSpec.layer.type.values)
